// Taken from MarioE's WorldEdit plugin
	...
	public static class Tools
	{
		public static List<byte> GetTileByName(string tile)
		{
			...
			if (byte.TryParse(tile, out ID) && ID < 149 &&
				!Main.tileFrameImportant[ID] && !MazesPlugin.InvalidTiles.Contains(ID))
			{
				return new List<byte> { ID };
			}

			List<byte> list = new List<byte>();
			foreach (KeyValuePair<string, byte> kv in MazesPlugin.TileNames)
			{
				if (kv.Key == tile)
				{
		...
		public static List<byte> GetWallByName(string wall)
		{
			...
			List<byte> list = new List<byte>();
			foreach (KeyValuePair<string, byte> kv in MazesPlugin.WallNames)
			{
				if (kv.Key == wall)
				{
					return new List<byte> { kv.Value };
		...
		{
            return File.Exists(Path.Combine("mazes", String.Format("clipboard-{0}.dat", plr)));
		}
		...
		{
			MazesPlugin.Players[plr].redoLevel = -1;
			MazesPlugin.Players[plr].undoLevel++;
            string path = Path.Combine("mazes", String.Format("undo-{0}-{1}.dat", plr, MazesPlugin.Players[plr].undoLevel));
			SaveWorldSection(x, y, x2, y2, path);

            foreach (string fileName in Directory.EnumerateFiles("mazes", String.Format("redo-{0}-*.dat", plr)))
			{
		...
		{
			MazesPlugin.Players[plr].undoLevel++;
            string redoPath = Path.Combine("mazes", String.Format("redo-{0}-{1}.dat", plr, MazesPlugin.Players[plr].redoLevel));
            string undoPath = Path.Combine("mazes", String.Format("undo-{0}-{1}.dat", plr, MazesPlugin.Players[plr].undoLevel));
			MazesPlugin.Players[plr].redoLevel--;
